...
// ExtractPackageFrom extracts the applied configuration owned by fieldManager from
// package for the specified subresource. Pass an empty string for subresource to extract 
// the main resource. Common subresources include "status", "scale", etc.
// package must be a unmodified Package API object that was retrieved from the Kubernetes API.
// ExtractPackageFrom provides a way to perform a extract/modify-in-place/apply workflow.
// Note that an extracted apply configuration will contain fewer fields than what the fieldManager previously
// applied if another fieldManager has updated or force applied any of the previously applied fields.
func ExtractPackageFrom(package *corev1alpha1.Package, fieldManager string, subresource string) (*PackageApplyConfiguration, error) {
	b := &PackageApplyConfiguration{}
	err := managedfields.ExtractInto(package, internal.Parser().Type("run.package-operator.apis.core.v1alpha1.Package"), fieldManager, b, subresource)
	if err != nil {
		return nil, err
	}
	b.WithName(package.Name)
	b.WithNamespace(package.Namespace)
...
// ExtractPackage extracts the applied configuration owned by fieldManager from
// package. If no managedFields are found in package for fieldManager, a
// PackageApplyConfiguration is returned with only the Name, Namespace (if applicable),
// APIVersion and Kind populated. It is possible that no managed fields were found for because other
// field managers have taken ownership of all the fields previously owned by fieldManager, or because
// the fieldManager never owned fields any fields.
// package must be a unmodified Package API object that was retrieved from the Kubernetes API.
// ExtractPackage provides a way to perform a extract/modify-in-place/apply workflow.
// Note that an extracted apply configuration will contain fewer fields than what the fieldManager previously
// applied if another fieldManager has updated or force applied any of the previously applied fields.
func ExtractPackage(package *corev1alpha1.Package, fieldManager string) (*PackageApplyConfiguration, error) {
	return ExtractPackageFrom(package, fieldManager, "")
}

// ExtractPackageStatus extracts the applied configuration owned by fieldManager from
// package for the status subresource.
func ExtractPackageStatus(package *corev1alpha1.Package, fieldManager string) (*PackageApplyConfiguration, error) {
	return ExtractPackageFrom(package, fieldManager, "status")
}
