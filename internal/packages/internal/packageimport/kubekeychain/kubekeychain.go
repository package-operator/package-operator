...
func FromServiceAccountPullSecrets(ctx context.Context, uncachedClient client.Client) (authn.Keychain, error) {
	// Should we validate this?
	serviceAccountName := types.NamespacedName{
		Namespace: os.Getenv("PKO_NAMESPACE"),
		Name:      os.Getenv("PKO_SERVICE_ACCOUNT"),
	}

	secrets, err := resolveSecrets(ctx, serviceAccountName, uncachedClient)
	if err != nil {
		return nil, err
	}
