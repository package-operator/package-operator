...
func TestResolveNothing(t *testing.T) {
	...
	r := packageresolving.BuildResolver{
		Loader: func(_ context.Context, _ []manifests.PackageManifestRepository) (*packages.MultiRepositoryIndex, error) {
			return idx, nil
		},
	}
	err := r.Solve()
...
func TestResolveBasic(t *testing.T) {
	...
	require.NoError(t, idx.Add(ctx, packages.Entry{
		...
		RepositoryEntry: &manifests.RepositoryEntry{
			Data: manifests.RepositoryEntryData{
				Image: "image", Digest: "digest", Name: "deppkg", Versions: []string{"1.0.0", "2.0.0", "3.0.0"},
			},
		},
	}))
	...
	expected := manifests.PackageManifestLockDependency{
		Name: "depname", Image: "image", Digest: "digest", Version: "3.0.0",
	}
...
func TestResolveNoCandidatesSpecificVersion(t *testing.T) {
	...
	pkg := &manifests.PackageManifest{
		...
		Spec: manifests.PackageManifestSpec{
			Dependencies: []manifests.PackageManifestDependency{
				{Image: &manifests.PackageManifestDependencyImage{Name: "depname", Package: "deppkg.deprepo", Range: ">=3.0.0"}},
			},
		},
	}
