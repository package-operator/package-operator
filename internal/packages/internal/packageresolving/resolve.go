...
func (r *BuildResolver) AddManifest(
	ctx context.Context, pkg *manifests.PackageManifest,
) (*[]manifests.PackageManifestLockDependency, error) {
	...
	for _, dep := range pkg.Spec.Dependencies {
		if dep.Image.Range != "" {
			...
			scope.Constrainers = append(scope.Constrainers,
				dependOnFQDNVersion(dep.Image.Package, semver.MustNewConstraint("x-x")),
			)
		}
	}
	...
	for _, entry := range idx.ListAllEntries() {
		...
		for _, verStr := range entry.Data.Versions {
			...
			if slices.ContainsFunc(scope.Candidates, chk) {
				return nil, fmt.Errorf("%w version %v of package %s exists multiple times in repository entry",
					ErrRepositoryInconsistent, newCandidate.Data.version, newCandidate.Data.fqdn,
				)
			}
