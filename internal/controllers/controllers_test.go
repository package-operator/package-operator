...
// Regression: AddDynamicCacheLabel previously used client.Merge which
// serialized the entire object as a JSON merge patch. When called with a
// desired object (e.g. from CreateCollisionError), this overwrote spec,
// owner references, and annotations on the pre-existing cluster object,
// bypassing collision protection.
func TestAddDynamicCacheLabel_PatchContainsOnlyLabel(t *testing.T) {
	t.Parallel()
	...
	assert.Contains(t, patchStr, constants.DynamicCacheLabel)
	assert.NotContains(t, patchStr, "desired-key",
		"patch must not contain spec fields from the desired object")
	assert.NotContains(t, patchStr, "ownerReferences",
		"patch must not contain owner references from the desired object")
	assert.NotContains(t, patchStr, "should-not-be-in-patch",
		"patch must not contain annotations from the desired object")
	assert.NotContains(t, patchStr, "evil-owner",
		"patch must not contain owner references from the desired object")
}
