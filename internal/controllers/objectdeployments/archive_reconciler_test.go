...
func Test_ArchivalReconciler(t *testing.T) {
	...
	t.Run(
		"when the latest revision becomes available, all intermediate revisions are paused first", func(t *testing.T) {
			t.Parallel()
			testPauseAndArchivalWhenLatestIsSpecAvailable(t, false)
		})
	...
	t.Run(
		...
		func(t *testing.T) {
			t.Parallel()
			testPauseAndArchivalWhenLatestIsSpecAvailable(t, true)
		})
...
func testPauseAndArchivalWhenLatestIsSpecAvailable(t *testing.T, alreadyPaused bool) {
	t.Helper()
	// setup Objectdeployment
	objectDeployment := &adaptermocks.ObjectDeploymentMock{}
	revisionLimit := int32(10)
...
func newObjectSetMock(
	...
	mock.On("IsSpecAvailable").Return(isAvailable)
	mock.On("IsSpecArchived").Return(isArchived)
