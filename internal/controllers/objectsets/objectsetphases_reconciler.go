...
func (r *objectSetPhasesReconciler) Reconcile(
	...
	log = log.WithName("objectSetPhasesReconciler")
	log.Info("reconcile")
...
func (r *objectSetPhasesReconciler) reconcile(
	...
	cache, err := r.cacheManager.GetWithUser(
		ctx,
	...
	phaseReconciler := r.phaseReconcilerFactory(cache)
...
func (r *objectSetPhasesReconciler) Teardown(
	...
	if err := r.cacheManager.FreeWithUser(
		ctx,
		constants.StaticCacheOwner(),
		objectSet,
	); err != nil {
