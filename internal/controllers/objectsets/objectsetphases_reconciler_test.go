...
func TestObjectSetPhasesReconciler_Reconcile(t *testing.T) {
	...
	prepare := func() *prepared {
		...
		cacheManager.On("GetWithUser", mock.Anything, mock.Anything, mock.Anything, mock.Anything).
			Return(accessor, nil)
	...
	t.Run("Teardown", func(t *testing.T) {
		...
		for i := range tests {
			...
			t.Run(test.name, func(t *testing.T) {
				...
				p.cacheManager.On("FreeWithUser", mock.Anything, mock.Anything, os).Return(nil)
				...
				if test.firstTeardownFinish {
					...
					p.cacheManager.AssertCalled(t, "FreeWithUser", mock.Anything, mock.Anything, os)
				} else {
					p.cacheManager.AssertNotCalled(t, "FreeWithUser", mock.Anything, mock.Anything, os)
				}
			})
		}
	})
