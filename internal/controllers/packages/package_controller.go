...
const (
	...
	pkoManagerDeploymentName    = "package-operator-manager"
	pkoTeardownJobName          = "package-operator-teardown-job"
	pkoTeardownJobContainerName = "teardown"
)
...
func (c *GenericPackageController) handleDeletion(ctx context.Context, pkg adapters.GenericPackageAccessor) error {
	...
	if pkg.ClientObject().GetName() != pkoClusterPackageName {
		return nil
	}
	...
	if err := c.client.Get(ctx, types.NamespacedName{Namespace: pkoNamespace, Name: pkoManagerDeploymentName}, &managerDpl); err != nil {
		return fmt.Errorf("get pko manager deployment: %w", err)
	}

	job := batchv1.Job{
		ObjectMeta: metav1.ObjectMeta{Name: pkoTeardownJobName, Namespace: managerDpl.Namespace},
			...
			Template: corev1.PodTemplateSpec{
				Spec: corev1.PodSpec{
					...
					Containers: []corev1.Container{
						{
							Name:  pkoTeardownJobContainerName,
