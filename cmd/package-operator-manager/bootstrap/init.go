...
func (init *initializer) ensureUpdatedPKO(ctx context.Context) (bool, error) {
	...
	if bootstrapClusterPackage.Spec.Image != existingClusterPackage.Spec.Image {
		log.V(1).Info("image has been updated",
	...
	if !equality.Semantic.DeepEqual(
		...
		log.V(1).Info("patching PackageOperator ClusterPackage")
	...
	if isAvailable {
		...
		log.V(1).Info("PackageOperator is available")
		return false, nil
	}
	log.V(1).Info("PackageOperator is NOT available")
	return true, nil
}
...
func (init *initializer) ensureCRDs(ctx context.Context, crds []unstructured.Unstructured) error {
	...
	for _, crd := range crds {
		...
		log.V(1).Info("ensuring CRD", "name", crd.GetName())
